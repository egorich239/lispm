...
extern const char _binary_captures_data_txt_start[];
extern const char _binary_captures_data_txt_end[];

LispmObj stack[4 * 1024 * 1024];
char strings[16 * 1024 * 1024];
unsigned htable[4 * 1024 * 1024];

static int sym_equal(LispmObj exp, LispmObj act) {
  if (lispm_obj_is_atom(act) && lispm_obj_is_atom(exp)) return exp == act;
  if (lispm_obj_is_cons(act) && lispm_obj_is_cons(exp)) {
    LispmObj *aa = lispm_obj_unpack(exp), *bb = lispm_obj_unpack(act);
    if (!sym_equal(aa[0], bb[0])) return 0;
    return sym_equal(aa[1], bb[1]);
  }
  if (lispm_obj_is_triplet(act) && lispm_obj_is_cons(exp)) {
    LispmObj *aa = lispm_obj_unpack(exp), *bb = lispm_obj_unpack(act);
    LispmObj parms, body;
    aa = lispm_obj_unpack(aa[1]), parms = aa[0], *aa = aa[1];
    aa = lispm_obj_unpack(aa[1]), body = aa[0];
...
static int sym_equal_capture(LispmObj expected, LispmObj actual) {
  LispmObj *cons, exp, name_exp, value_exp, act, name_act, value_act;
  /* VM stores catpures in pairs, not in lists of two elements,
     whereas expectations are parsed as lists of two elements */
  while (lispm_obj_is_cons(expected) && lispm_obj_is_cons(actual)) {
    cons = lispm_obj_unpack(expected), exp = cons[0], expected = cons[1];
    cons = lispm_obj_unpack(exp), name_exp = cons[0], value_exp = cons[1];
    cons = lispm_obj_unpack(value_exp), value_exp = cons[0];
    cons = lispm_obj_unpack(actual), act = cons[0], actual = cons[1];
    cons = lispm_obj_unpack(act), name_act = cons[0], value_act = cons[1];
    if (!sym_equal(name_exp, name_act) || !sym_equal(value_exp, value_act)) return 0;
  }
  if (!lispm_obj_is_nil(expected) || !lispm_obj_is_nil(actual)) return 0;
  return 1;
}

static int failure;
static LispmObj next_lambda;
static void trace_lambda(LispmObj lambda) {
  lispm_print_short(lambda);
  fprintf(stderr, "\n");
  if (lispm_obj_is_nil(next_lambda)) {
  ...
  LispmObj act = lispm_obj_unpack(lambda)[0];
  LispmObj *cons = lispm_obj_unpack(next_lambda), exp = cons[0];
  next_lambda = cons[1];
...
struct Lispm lispm = {
    ...
    .program_end = _binary_captures_data_txt_end,
    .pc = _binary_captures_data_txt_start,
