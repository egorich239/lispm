...
static Obj LIST(Obj a) { return a; }
...
static Obj CONS_UNPACK(Obj a, unsigned offs) {
  ...
  return lispm_obj_unpack(arg)[offs];
}
...
static Obj ATOM(Obj a) {
  ...
  return lispm_obj_is_atom(arg) ? lispm_obj_from_builtin(BUILTIN_T) : LISPM_SYM_NIL;
}
static Obj EQ(Obj a) {
  Obj xy[2];
  LISPM_EVAL_CHECK(lispm_list_scan(xy, a, 2) == 2, a, panic, "two arguments expected, got: ", a);
  return lispm_obj_is_atom(xy[0]) && xy[0] == xy[1] ? lispm_obj_from_builtin(BUILTIN_T) : LISPM_SYM_NIL;
}
