...
static Sym PROGRAM(Sym e) {
  ...
  return lispm_alloc_span(LISPM_PAGE_PROGRAM, lispm_make_shortnum(0),
...
static Sym CHARS(Sym a) {
  ...
  while (l--)
    res = lispm_alloc_cons(lispm_make_shortnum(*--p), res);
  return res;
}
static Sym BAND(Sym a) {
  ...
  return lispm_shortnum_band(p, q);
}
static Sym BNOT(Sym a) {
  a = lispm_evquote(a);
  EVAL_CHECK(lispm_sym_is_shortnum(a), LISPM_ERR_EVAL);
  return lispm_shortnum_bnot(a);
}
static Sym ADD(Sym a) {
  ...
  EVAL_CHECK(lispm_sym_is_shortnum(p) && lispm_sym_is_shortnum(q), LISPM_ERR_EVAL);
  int overflow;
  Sym r = lispm_shortnum_add(p, q, &overflow);
...
static Sym MUL(Sym a) {
  ...
  EVAL_CHECK(lispm_sym_is_shortnum(p) && lispm_sym_is_shortnum(q), LISPM_ERR_EVAL);
  int overflow;
  Sym r = lispm_shortnum_mul(p, q, &overflow);
  EVAL_CHECK(!overflow, LISPM_ERR_EVAL);
  return r;
}
static Sym SUB(Sym a) {
  ...
  EVAL_CHECK(lispm_sym_is_shortnum(p) && lispm_sym_is_shortnum(q), LISPM_ERR_EVAL);
  int overflow;
  Sym r = lispm_shortnum_sub(p, q, &overflow);
  EVAL_CHECK(!overflow, LISPM_ERR_EVAL);
  return r;
}
static Sym SPAN(Sym args) {
  ...
  int o;
  len = !lispm_sym_is_nil(len) ? lispm_evquote(len) : lispm_shortnum_sub(old_len, start, &o);
  EVAL_CHECK(len <= old_len, LISPM_ERR_EVAL);
  return lispm_alloc_span(page, lispm_shortnum_add(offs, start, &o), len);
}
